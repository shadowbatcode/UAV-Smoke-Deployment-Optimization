...
min_drop_interval = 1.0  # 投放间隔至少1s（新增）
...
def is_fully_shielded_multi(missile_pos, smoke_centers, smoke_r, target_samples):
    """判定是否完全遮蔽（多烟幕弹联合）"""
...
def fitness_function(params, target_samples):
    """改进适应度计算，支持多枚烟幕弹"""
    theta, v, t1_1, t2_1, t1_2, t2_2, t1_3, t2_3 = params

    # ---------------- 约束检查 ----------------
    if not (70.0 <= v <= 140.0) or any(t < 0 for t in [t1_1, t2_1, t1_2, t2_2, t1_3, t2_3]):
        return 0.0 + np.random.uniform(-0.1, 0)
    if t1_2 < t1_1 + min_drop_interval or t1_3 < t1_2 + min_drop_interval:
        return 0.0 + np.random.uniform(-0.1, 0)
    ...
    # 第一枚
    drop_point1 = fy1_init_pos + v * t1_1 * uav_dir
    det_xy1 = drop_point1[:2] + v * t2_1 * uav_dir[:2]
    det_z1 = drop_point1[2] - 0.5 * g * t2_1**2
    if det_z1 < 5.0:
        return 0.0 + np.random.uniform(-0.5, 0)
    det_point1 = np.array([det_xy1[0], det_xy1[1], det_z1])
    t_det1 = t1_1 + t2_1

    # 第二枚
    drop_point2 = fy1_init_pos + v * t1_2 * uav_dir
    det_xy2 = drop_point2[:2] + v * t2_2 * uav_dir[:2]
    det_z2 = drop_point2[2] - 0.5 * g * t2_2**2
    if det_z2 < 5.0:
        return 0.0 + np.random.uniform(-0.5, 0)
    det_point2 = np.array([det_xy2[0], det_xy2[1], det_z2])
    t_det2 = t1_2 + t2_2

    # 第三枚
    drop_point3 = fy1_init_pos + v * t1_3 * uav_dir
    det_xy3 = drop_point3[:2] + v * t2_3 * uav_dir[:2]
    det_z3 = drop_point3[2] - 0.5 * g * t2_3**2
    if det_z3 < 5.0:
        return 0.0 + np.random.uniform(-0.5, 0)
    det_point3 = np.array([det_xy3[0], det_xy3[1], det_z3])
    t_det3 = t1_3 + t2_3

    # ---------------- 确定整体时间范围 ----------------
    t_dets = [t_det1, t_det2, t_det3]
    t_start = min(t_dets)
    t_smoke_ends = [t_det + smoke_param["valid_time"] for t_det in t_dets]
    t_end = min(max(t_smoke_ends), missile_arrival_time)
    if t_start >= t_end:
        return 0.0 + np.random.uniform(-0.1, 0)
    ...
    # ---------------- 时间序列 ----------------
    t_list = get_adaptive_time_steps(t_start, t_end, event_time)
    ...
    for t in t_list:
        if prev_t is not None:
            ...
            for i, t_det in enumerate(t_dets):
                if t_det <= t < t_det + smoke_param["valid_time"]:
                    sink_time = t - t_det
                    smoke_z = det_points[i][2] - smoke_param["sink_speed"] * sink_time
                    if smoke_z < 2.0:  # 烟幕过低跳过
                        continue
                    smoke_center = np.array([det_points[i][0], det_points[i][1], smoke_z])
            ...
            # 联合遮蔽判定
            if is_fully_shielded_multi(missile_pos, active_smoke_centers, smoke_param["r"], target_samples):
...
def count_shielded(missile_pos, smoke_centers, r, target_samples):
    """统计某一时刻目标是否被多枚烟幕遮蔽"""
    cover_count = 0
    for center in smoke_centers:
        if is_fully_shielded_multi(missile_pos, center, r, target_samples):
            cover_count += 1
    return cover_count
    ...
    def optimize(self):
        ...
        for iter in range(self.max_iter):
            ...
            w = self.w_start - (self.w_start - self.w_end) * (iter / self.max_iter) \
                + (1 - self.w_end) * mean_fitness / (np.max(self.pbest_fitness) + eps)            
            # 并行计算所有粒子的适应度
...
if __name__ == "__main__":
    ...
    bounds = [
        ...
        (70.0, 140.0),         # v: 无人机速度
        (0.0, 80.0),           # t1_1: 第一枚投放延迟
        (0.0, 25.0),           # t2_1: 第一枚起爆延迟
        (0.0, 80.0),           # t1_2: 第二枚投放延迟
        (0.0, 25.0),           # t2_2: 第二枚起爆延迟
        (0.0, 80.0),           # t1_3: 第三枚投放延迟
        (0.0, 25.0)            # t2_3: 第三枚起爆延迟
    ]
    ...
    pso = ParticleSwarmOptimizer(
        ...
        bounds=bounds,
        num_particles=80,       # 增加粒子数量以应对更高维度
        max_iter=200,           # 增加迭代次数
        c1=1.5,                 # 认知系数
        c2=1.5,                 # 社会系数
        w_start=0.9,            # 初始惯性权重
    ...
    # 提取最优解
    theta_opt, v_opt, t1_1_opt, t2_1_opt, t1_2_opt, t2_2_opt, t1_3_opt, t2_3_opt = best_params
    ...
    # 第一枚
    drop_point1_opt = fy1_init_pos + v_opt * t1_1_opt * uav_dir_opt
    det_xy1_opt = drop_point1_opt[:2] + v_opt * t2_1_opt * uav_dir_opt[:2]
    det_z1_opt = drop_point1_opt[2] - 0.5 * g * t2_1_opt**2
    det_point1_opt = np.array([det_xy1_opt[0], det_xy1_opt[1], det_z1_opt])
    t_det1_opt = t1_1_opt + t2_1_opt
    
    # 第二枚
    drop_point2_opt = fy1_init_pos + v_opt * t1_2_opt * uav_dir_opt
    det_xy2_opt = drop_point2_opt[:2] + v_opt * t2_2_opt * uav_dir_opt[:2]
    det_z2_opt = drop_point2_opt[2] - 0.5 * g * t2_2_opt**2
    det_point2_opt = np.array([det_xy2_opt[0], det_xy2_opt[1], det_z2_opt])
    t_det2_opt = t1_2_opt + t2_2_opt
    
    # 第三枚
    drop_point3_opt = fy1_init_pos + v_opt * t1_3_opt * uav_dir_opt
    det_xy3_opt = drop_point3_opt[:2] + v_opt * t2_3_opt * uav_dir_opt[:2]
    det_z3_opt = drop_point3_opt[2] - 0.5 * g * t2_3_opt**2
    det_point3_opt = np.array([det_xy3_opt[0], det_xy3_opt[1], det_z3_opt])
    t_det3_opt = t1_3_opt + t2_3_opt
    ...
    # 输出结果
    print("\n" + "="*80)
    print("【粒子群优化结果 - 最优烟幕弹投放策略（3枚）】")
    print(f"优化耗时: {elapsed_time:.2f} 秒")
    print(f"1. 无人机飞行方向角: {theta_opt:.6f} rad ({np.degrees(theta_opt):.2f}°)")
    print(f"2. 无人机飞行速度: {v_opt:.4f} m/s")
    print(f"\n第一枚烟幕弹:")
    print(f"  投放延迟时间: {t1_1_opt:.4f} s")
    print(f"  起爆延迟时间: {t2_1_opt:.4f} s")
    print(f"  投放点坐标: {drop_point1_opt.round(4)}")
    print(f"  起爆点坐标: {det_point1_opt.round(4)}")
    print(f"  烟幕有效窗口: [{t_det1_opt:.2f}s, {t_det1_opt+20:.2f}s]")
    print(f"\n第二枚烟幕弹:")
    print(f"  投放延迟时间: {t1_2_opt:.4f} s")
    print(f"  起爆延迟时间: {t2_2_opt:.4f} s")
    print(f"  投放点坐标: {drop_point2_opt.round(4)}")
    print(f"  起爆点坐标: {det_point2_opt.round(4)}")
    print(f"  烟幕有效窗口: [{t_det2_opt:.2f}s, {t_det2_opt+20:.2f}s]")
    print(f"\n第三枚烟幕弹:")
    print(f"  投放延迟时间: {t1_3_opt:.4f} s")
    print(f"  起爆延迟时间: {t2_3_opt:.4f} s")
    print(f"  投放点坐标: {drop_point3_opt.round(4)}")
    print(f"  起爆点坐标: {det_point3_opt.round(4)}")
    print(f"  烟幕有效窗口: [{t_det3_opt:.2f}s, {t_det3_opt+20:.2f}s]")
    print(f"\n有效遮蔽总时长:")
    print(f"  优化阶段: {best_fitness:.6f} s")
    print(f"  验证阶段: {verify_fitness:.6f} s")
    print("="*80)

    # 保存结果到Excel（假设模板结构为：无人机ID, 烟幕弹序号, 方向角, 速度, 投放时间, 起爆时间, 投放点x,y,z, 起爆点x,y,z 等）
    data = {
        '无人机ID': ['FY1', 'FY1', 'FY1'],
        '烟幕弹序号': [1, 2, 3],
        '飞行方向角 (rad)': [theta_opt, theta_opt, theta_opt],
        '飞行速度 (m/s)': [v_opt, v_opt, v_opt],
        '投放延迟时间 (s)': [t1_1_opt, t1_2_opt, t1_3_opt],
        '起爆延迟时间 (s)': [t2_1_opt, t2_2_opt, t2_3_opt],
        '投放点x': [drop_point1_opt[0], drop_point2_opt[0], drop_point3_opt[0]],
        '投放点y': [drop_point1_opt[1], drop_point2_opt[1], drop_point3_opt[1]],
        '投放点z': [drop_point1_opt[2], drop_point2_opt[2], drop_point3_opt[2]],
        '起爆点x': [det_point1_opt[0], det_point2_opt[0], det_point3_opt[0]],
        '起爆点y': [det_point1_opt[1], det_point2_opt[1], det_point3_opt[1]],
        '起爆点z': [det_point1_opt[2], det_point2_opt[2], det_point3_opt[2]],
        '有效遮蔽时长 (s)': [verify_fitness, verify_fitness, verify_fitness]  # 总时长重复填入或调整
    }
